...
export class MosaicConfigurationsUtils {
    ...
    static saveConfigurationToFile(configuration: MosaicConfiguration): void {
        if (browser) {
            ...
            let configurationName = prompt('Please enter configuration name', currentDate.toJSON());
            ...
            if (browser) {
                ...
                input.onchange = (event) => {
                    ...
                    if (file) {
                        ...
                        reader.onload = (event) => {
                            try {
                                const configuration = JSON.parse(event.target?.result as string) as MosaicConfiguration;
    ...
    static saveConfigurationAsDefault(configuration: MosaicConfiguration): void {
        if (browser)
            localStorage.setItem(MosaicConfigurationsUtils.DEFAULT_CONFIGURATION_NAME, JSON.stringify(configuration));
    }

    static loadDefaultConfiguration(): MosaicConfiguration | null {
        if (browser) {
            const configuration = localStorage.getItem(MosaicConfigurationsUtils.DEFAULT_CONFIGURATION_NAME);
            if (configuration) {
