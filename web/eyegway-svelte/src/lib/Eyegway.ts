...
export class EyegwayHubClient {
    ...
    async historySize(): Promise<number> {
        const response = await fetch(
            this.buildUrl(`/hubs/${this.hubName}/history_size`)
        );
    ...
    async clearHistory() {
        await fetch(
            this.buildUrl(`/hubs/${this.hubName}/clear_history`),
            { method: 'POST' }
        );
    }
    ...
    async bufferSize(): Promise<number> {
        const response = await fetch(
            this.buildUrl(`/hubs/${this.hubName}/buffer_size`)
        );
        return response.json();
    }
    ...
    async clearBuffer() {
        await fetch(
            this.buildUrl(`/hubs/${this.hubName}/clear_buffer`),
            { method: 'POST' }
        );
    }
    ...
    async last(offset: number = 0): Promise<any | null> {
        const response = await fetch(
            this.buildUrl(`/hubs/${this.hubName}/last?offset=${offset}`)
        );
    ...
    async pop(timeout: number = 1): Promise<any | null> {
        const response = await fetch(
            this.buildUrl(`/hubs/${this.hubName}/pop?timeout=${timeout}`)
        );
        if (response.status === 200) {
            return this.decodeData(await response.arrayBuffer());
        } else {
    ...
    async push(data: any): Promise<any> {
        await fetch(
            this.buildUrl(`/hubs/${this.hubName}/push`),
            {
                method: 'POST',
